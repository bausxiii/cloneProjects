...
symbol in the plaintext and each symbol in the ciphertext.''')
    ...
    while True:  # Keepasking until the user enters e or d.
    ...
    while True:  # Keep asking until the user enters a valid key.
        print(f'Please specify the key to use. ({len(LETTERS)} chars long)')
        ...
        if response == 'RANDOM' and not myMode == 'decrypt':
    ...
    try:
        ...
        pass  # Do nothing if pyperclip wasnt installed.
...
def encryptMessage(message, key):
    """Encryp the message using the key."""
